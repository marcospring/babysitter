...
<script type="text/javascript">
						...
						function(b) {
							if (b) {
								var currentUserId = '${sessionUser.id}';/*当前登录用户的ID*/
									...
									$
											.post(
													'${pageContext.request.contextPath}/manage/level/delete.html',
													{
														id : id
													},
													function(result) {
	...
	function editFun(id) {
		...
		parent.$
				.modalDialog({
					...
					width : 690,
					height : 170,
					href : '${pageContext.request.contextPath}/manage/level/goEdit.html?id='
							+ id,
					buttons : [ {
						text : '编辑',
						handler : function() {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 690,
			height : 170,
			href : '${pageContext.request.contextPath}/manage/level/goAdd.html',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
