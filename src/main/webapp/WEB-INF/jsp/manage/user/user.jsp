...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/manage/user/list.html',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
						...
						function(b) {
							if (b) {
								...
								if (currentUserId != id) {
									...
									$
											.post(
													'${pageContext.request.contextPath}/manage/user/delete.html',
													{
														id : id
													},
													function(result) {
	...
	function editFun(id) {
		...
		parent.$
				.modalDialog({
					...
					width : 500,
					height : 300,
					href : '${pageContext.request.contextPath}/manage/user/goEdit.html?id='
							+ id,
					buttons : [ {
						text : '编辑',
						handler : function() {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 500,
			height : 300,
			href : '${pageContext.request.contextPath}/manage/user/goAdd.html',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
