...
public class PropertiesUtils {
	...
	public static Properties loadProperties(String... resourcesPaths)
		...
		for (String location : resourcesPaths) {
			...
			try {
				...
				log.info("Could not load properties from classpath:" + location
						+ ": " + ex.getMessage());
			} finally {
				if (is != null) {
					is.close();
	...
	// staticProperties =
	// PropertiesUtils.loadProperties("/config/static.properties");
	// }
	// String value = new
	// String((staticProperties.getProperty(key)).getBytes("ISO8859_1"),
	// "utf-8").trim();
	// log.debug(key + ":" + value);
	// return value;
	// } catch (Exception e) {
